...
async function callAI(model, messages, tools = null, toolChoice = 'auto') {
  ...
  // Google Gemini OpenAI-compatible endpoint logic
  let url = `${baseUrl}/chat/completions`;
  
  if (baseUrl.includes('generativelanguage.googleapis.com')) {
    // Standard Gemini-OpenAI bridge URL structure:
    // https://generativelanguage.googleapis.com/v1beta/openai/chat/completions?key=YOUR_API_KEY
    // We enforce the /v1beta/openai part if missing
    if (!baseUrl.includes('/v1beta/openai')) {
      baseUrl = 'https://generativelanguage.googleapis.com/v1beta/openai';
    }
    url = `${baseUrl}/chat/completions?key=${API_KEY()}`;
    
    // Ensure model name doesn't have 'models/' prefix for the OpenAI bridge
    modelName = model.replace(/^models\//, '');
    
    console.log(`[agentService] Gemini Request: URL=${url.replace(API_KEY(), '***')}, Model=${modelName}`);
  }

  // Update body with final modelName
  ...
  if (!res.ok) {
    ...
    console.error(`[agentService] AI API error (${res.status}) for URL ${url.replace(API_KEY(), '***')}: ${errorText}`);
...
async function runPlannerAgent(userMessage, history, selector) {
  const contextMsg = selector ? `[CONTEXT] The user has selected the element with CSS selector: "${selector}". Focus your changes on this element if applicable.` : '';
  const messages = [
...
async function runCoderAgent(plan, userMessage, fileTree, projectMeta, existingFiles, selector) {
  const contextMsg = selector ? `[CONTEXT] The user has selected the element with CSS selector: "${selector}". Ensure your changes affect this element specifically.` : '';
  const messages = [
 ...
 * @param {string}   [selector]  – Optional CSS selector of the element being edited
 * @returns {Promise<{explanation: string, actions: FileAction[]}>}
 */
export async function runAgentPipeline(projectId, userMessage, history, projectMeta, selector = null) {
  ...
  const plan = await runPlannerAgent(userMessage, history, selector);

  // 4. CoderAgent: generate file actions
  const coderResult = await runCoderAgent(plan, userMessage, fileTree, projectMeta, existingFiles, selector);
